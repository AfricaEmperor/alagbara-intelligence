...
Produce decision-grade intelligence while keeping evidence and uncertainty explicit.

BILINGUAL OUTPUT IS MANDATORY. Every human-readable value must contain BOTH languages in this exact pattern:
English text\nFR: Texte français
For array items, apply the same pattern to every item. English comes first; French is the faithful translation second. Do not omit either language.
...
{"pulse":"English text\\nFR: Texte français","facts":["English fact\\nFR: Fait français"],"unknowns":["English unknown\\nFR: Inconnue française"],"assumptions":["English assumption\\nFR: Hypothèse française"],"analysis":"English analysis\\nFR: Analyse française","recommendation":"English next step\\nFR: Prochaine étape en français","risks":["English risk\\nFR: Risque français"],"confidence":"low|medium|high"}`;
...
export async function POST(request) {
  ...
  try {
    ...
    if (!openaiKey) return json({ request_id: requestId, status: 'received', next: 'Intelligence processing pending / traitement de l’intelligence en attente' }, 202, headers);
    ...
    const raw = await callOpenAI(openaiKey, JSON.stringify({ request: { id: requestId, question, market: market || null, decision: decision || null, useful: useful || null }, recent_intelligence_memory: recent || [] }), model);
  ...
  } catch (error) { console.error('[intelligence-request]', error); return json({ error: error.message || 'Intelligence processing failed after request persistence / échec du traitement après enregistrement', request_id: requestId }, 502, headers); }
}

async function setStatus(supabase, id, nonce, status) { const { error } = await supabase.rpc('update_big_intelligence_request_status', { p_id: id, p_nonce: nonce, p_status: status }); if (error) throw new Error('Could not advance intelligence status to ' + status + ' / impossible de faire avancer le statut: ' + error.message); }
async function callOpenAI(apiKey, userContent, model) {
  const response = await fetch('https://api.openai.com/v1/responses', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${apiKey}` }, body: JSON.stringify({ model, input: [{ role: 'system', content: [{ type: 'input_text', text: ANALYSIS_SYSTEM }] }, { role: 'user', content: [{ type: 'input_text', text: userContent }] }], max_output_tokens: 1800 }) });
  if (!response.ok) { let detail = 'status ' + response.status; try { const data = await response.json(); if (data?.error?.message) detail = data.error.message; } catch (_) {} throw new Error('OpenAI request failed / requête OpenAI échouée — ' + detail); }
  const data = await response.json(); if (typeof data.output_text === 'string') return data.output_text; return (data.output || []).flatMap(item => item.content || []).filter(part => part.type === 'output_text').map(part => part.text).join('\n');
}
function parseAnalysis(text) {
  const clean = String(text || '').replace(/```json|```/g, '').trim();
  try { const value = JSON.parse(clean); if (!value || typeof value !== 'object' || Array.isArray(value)) return { ok: false, reason: 'model response was not an object / la réponse du modèle n’est pas un objet' }; for (const field of ['facts','unknowns','assumptions','risks']) if (!Array.isArray(value[field])) return { ok:false, reason: field+' must be an array / doit être un tableau' }; for (const field of ['pulse','analysis','recommendation']) if (typeof value[field] !== 'string' || !value[field].trim()) return { ok:false, reason: field+' must be a non-empty string / doit être une chaîne non vide' }; if (!['low','medium','high'].includes(value.confidence)) return {ok:false,reason:'confidence must be low, medium, or high / doit être low, medium ou high'}; return {ok:true,value}; } catch (error) { return {ok:false,reason:'model response was not valid JSON / réponse JSON invalide: '+error.message}; }
}
